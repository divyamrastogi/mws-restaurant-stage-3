...
const submitReview = (event) => {
  ...
  if (name == null || name == '' || rating == null || rating == '' || comments == null || comments == '') {
    Toast.showToast("You missed out on some fields.");
    return;
  }
  ...
  resetReview(nameField, ratingField, commentsField);
};

/**
 * Reset review form.
 */
const resetReview = (nameField, ratingField, commentsField) => {
  nameField.value = '';
  ratingField.value = '';
  commentsField.value = '';
};
...
const syncReviewsWithServer = () => {
  Promise.all(reviewsToBeSynced.map(review => {
    ...
    Toast.showToast('Reviews have been synced in the background.');
...
const isOffline = (event) => {
  Toast.showToast("You're offline.");
};
